...
func TestValidateEvidencePlanRequiresFeatureSnapshot(t *testing.T) {
	...
	good := `{"feature":"f","step":"plan","role":"big-thinker","status":"done","generatedAt":"` + time.Now().UTC().Format(time.RFC3339) + `","inputs":["docs/features/f.md","./docs/features/a.md","docs/plans/f.md"],"outputs":["docs/plans/f.md"],"edgeCases":[],"handoffTo":"orchestrator"}`
	...
	spec := `{"feature":"f","step":"plan","role":"feature-specialist","status":"done","generatedAt":"` + time.Now().UTC().Format(time.RFC3339) + `","inputs":["docs/features/a.md","docs/features/f.md","docs/plans/f.md"],"outputs":["specs/f.feature"],"edgeCases":["e"],"handoffTo":"orchestrator"}`
