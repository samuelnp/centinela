...
func TestValidateEvidencePlanRequiresFeatureSnapshot(t *testing.T) {
	...
	bad := `{"feature":"f","step":"plan","role":"big-thinker","status":"done","generatedAt":"` + time.Now().UTC().Format(time.RFC3339) + `","inputs":["docs/features/f.md"],"outputs":["o"],"edgeCases":[],"handoffTo":"orchestrator"}`
	os.WriteFile(path, []byte(bad), 0644) //nolint:errcheck
	err := ValidateEvidence(path, "f", "plan", RoleBigThinker)
	...
	good := `{"feature":"f","step":"plan","role":"big-thinker","status":"done","generatedAt":"` + time.Now().UTC().Format(time.RFC3339) + `","inputs":["docs/features/f.md","./docs/features/a.md"],"outputs":["o"],"edgeCases":[],"handoffTo":"orchestrator"}`
	...
	spec := `{"feature":"f","step":"plan","role":"feature-specialist","status":"done","generatedAt":"` + time.Now().UTC().Format(time.RFC3339) + `","inputs":["docs/features/a.md","docs/features/f.md"],"outputs":["o"],"edgeCases":["e"],"handoffTo":"orchestrator"}`
	os.WriteFile(sp, []byte(spec), 0644) //nolint:errcheck
	if err := ValidateEvidence(sp, "f", "plan", RoleFeatureSpecial); err != nil {
