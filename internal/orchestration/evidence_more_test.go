...
func TestValidateEvidenceErrorBranches(t *testing.T) {
	...
	if err := ValidateEvidence(JSONPath("f", RoleBigThinker), "f", "plan", RoleBigThinker, nil); err == nil {
...
func TestValidateEvidenceIncompleteAndEdgeCases(t *testing.T) {
	...
	if err := ValidateEvidence(path, "f", "plan", RoleBigThinker, nil); err == nil || !strings.Contains(err.Error(), "incomplete") {
	...
	if err := ValidateEvidence(q, "f", "tests", RoleQASeniorEngineer, nil); err == nil || !strings.Contains(err.Error(), "edgeCases") {
