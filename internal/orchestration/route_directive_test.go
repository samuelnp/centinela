...
func TestRoutingDirective_NamesUnroutedRolesFloorsAndStatics(t *testing.T) {
	...
	for _, want := range []string{
		...
		// E3: the label must say what the floor actually governs.
		"floors (routes only): planner>=balanced",
...
func TestRoutingDirective_MixedRolesAndConfiguredFloors(t *testing.T) {
	...
	floors := map[string]string{"ux-ui-specialist": "fast"}
...
// A recorded route the overlay will IGNORE leaves the decision open, so the
// role must reappear on the line rather than be silently counted as decided.
func TestRoutingDirective_UnhonoredRouteStillCountsAsUnrouted(t *testing.T) {
	roles := []Role{RoleGatekeeper}
	for _, tier := range []string{"fast", "turbo"} {
		line := RoutingDirective("f", roles, map[string]string{"gatekeeper": tier}, nil, RoleModels{})
		if !strings.Contains(line, "unrouted [gatekeeper]") {
			t.Fatalf("an ignored %q route must stay an open decision: %q", tier, line)
		}
	}
	honored := RoutingDirective("f", roles, map[string]string{"gatekeeper": "reasoning"}, nil, RoleModels{})
	if honored != "" {
		t.Fatalf("an honored route closes the decision, got %q", honored)
	}
}
