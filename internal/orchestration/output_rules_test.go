...
func TestValidateActionableOutputsRoleRules(t *testing.T) {
	...
	if err := validateActionableOutputs("x", "f", RoleBigThinker, []string{"summary only"}); err == nil || !strings.Contains(err.Error(), "real files") {
		t.Fatalf("expected missing file error, got %v", err)
	}
	if err := validateActionableOutputs("x", "f", RoleBigThinker, []string{"docs/plans/f.md"}); err != nil {
		t.Fatalf("expected big-thinker success, got %v", err)
	}
	if err := validateActionableOutputs("x", "f", RoleSeniorEngineer, []string{".workflow/f-senior-engineer.md"}); err == nil || !strings.Contains(err.Error(), "non-evidence implementation") {
		t.Fatalf("expected implementation output error, got %v", err)
	}
	if err := validateActionableOutputs("x", "f", RoleSeniorEngineer, []string{"internal/demo/file.go"}); err != nil {
		t.Fatalf("expected senior-engineer success, got %v", err)
	}
	if err := validateActionableOutputs("x", "f", RoleQASeniorEngineer, []string{"tests/unit/f_test.go"}); err == nil || !strings.Contains(err.Error(), "edge-cases") {
		t.Fatalf("expected qa edge-case error, got %v", err)
	}
	if err := validateActionableOutputs("x", "f", RoleQASeniorEngineer, []string{"tests/unit/f_test.go", ".workflow/f-edge-cases.md"}); err != nil {
		t.Fatalf("expected qa success, got %v", err)
	}
	if err := validateActionableOutputs("x", "f", RoleDocsSpecialist, []string{"summary only"}); err != nil {
