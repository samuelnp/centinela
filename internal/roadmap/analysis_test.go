...
func TestValidateAnalysisInvalidBranches(t *testing.T) {
	...
	os.WriteFile(RoadmapAnalysisFile, []byte(`{"role":"qa","features":[{"name":"user","dependsOn":[]}]}`), 0644) //nolint:errcheck
	...
	os.WriteFile(RoadmapAnalysisFile, []byte(`{"role":"senior-product-manager","features":[{"name":"ghost","dependsOn":[]}]}`), 0644) //nolint:errcheck
	...
	os.WriteFile(RoadmapAnalysisFile, []byte(`{"role":"senior-product-manager","features":[{"name":"user","dependsOn":["ghost"]}]}`), 0644) //nolint:errcheck
	if err := ValidateAnalysis(r); err == nil || !strings.Contains(err.Error(), "depends on unknown") {
