...
func TestRoadmapContextFallbackBranches(t *testing.T) {
	...
	// Option B: deps live on roadmap.json, not the analysis. Declare dependsOn there.
	os.WriteFile(roadmap.RoadmapFile, []byte(`{"phases":[{"name":"P1","features":[{"name":"dep"},{"name":"sib1"},{"name":"sib2"},{"name":"f","dependsOn":["dep"]}]}]}`), 0644) //nolint:errcheck
