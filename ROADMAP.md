...
- **workflow-state-atomic-save** — workflow.Save is an unlocked, non-atomic whole-file WriteFile; concurrent route set + complete lose updates and a killed write bricks the feature state *(deferred 2026-07-30T18:12:16Z · dynamic-model-routing/edge-case-tester)*
- **workflow-state-schema-version** — No schema version on .workflow/*.json: an older binary silently drops modelRoutes (and any newer field) on every load-save round-trip *(deferred 2026-07-30T18:12:16Z · dynamic-model-routing/edge-case-tester)*
