...
- **profile-plan-advisor-knob-inert** — ProfileKnobs.PlanAdvisorMode is never read by any production caller: planadvisor.Directive reads cfg.Workflow.PlanAdvisorMode directly and NormalizePlanAdvisorMode defaults to missing_info, so strict's documented always is inert *(deferred 2026-08-03T08:35:24Z · guided-by-default/planner)*
- **profile-step-gating-knob-inert** — ProfileKnobs.StepGating is never read: hookpolicy/prewrite.go branches on EffectiveProfile == ProfileOutcome directly, so a future profile with StepGating=false would silently still step-gate *(deferred 2026-08-03T08:35:24Z · guided-by-default/planner)*
- **profile-display-unused-and-now-misleading** — workflow.DisplayProfile has no production caller and, since the guided default is state-dated on ProfileContract rather than on EnforcementProfile, it now reports strict for every guided-by-default workflow — delete it or make it contract-aware before any surface adopts it *(deferred 2026-08-03T12:07:37Z · guided-by-default/senior-engineer)*
- **provenance-local-default-ignores-capability-profile-override** — ProfileProvenance hardcodes strict on the local-default branch while EffectiveProfile honours [orchestration.capability_profiles] limited, so status says strict and verdict says guided for one state file (pre-existing on main, but the round-2 docstring now claims all four resolvers implement the same rule) *(deferred 2026-08-03T15:10:42Z · guided-by-default/gatekeeper)*
- **deferred-findings-spec-scenario-uncovered** — specs/deferred-findings-roadmap-capture.feature 'Defer auto-resolves --source from worktree CWD when flag is omitted' has no acceptance marker; this branch's edit to that spec pulls the pre-existing gap into diff scope and the spec-traceability gate now warns on it *(deferred 2026-08-03T15:10:42Z · guided-by-default/gatekeeper)*
- **harness-model-alias-unverified** — Centinela records tier->model IDs (e.g. claude-opus-4-7) but the orchestrator can only pass harness aliases (opus/sonnet/haiku); nothing verifies the alias resolves to the configured ID, so routing records may not reflect the model actually used — add a startup/doctor check comparing configured IDs against harness-resolved models *(deferred 2026-08-03T20:45:26Z · guided-by-default/orchestrator)*
- **resolvestart-lacks-failclosed-value-guard** — workflow.ResolveStart is a fourth profile resolver whose default branch returns guided unconditionally: on a nil or zero-valued config it answers guided where EffectiveProfile and ProjectDefaultProfile both answer strict (verified empirically). Not live — start.go propagates the Load error and hook_autostart declines — but profile.go documents the guard as living at the decision so all resolvers agree on any input, and ResolveStart is the surface that writes the once-and-for-life profileContract pin; add the ResolvedByLoad/LoadFailed guard there too *(deferred 2026-08-03T21:08:42Z · guided-by-default/gatekeeper)*
- **setup-rung-content-fault-diverges-from-start** — hook setup's grading rungs ask existence + not-provisional (evaluatedArtifact) while start asks ValidateAnalysis/ValidateQuality, which also check role, feature coverage and JSON shape; on a strict tree whose artifacts are present, non-provisional and content-faulty (or unparseable — ArtifactIsProvisional returns false for those) hook setup reaches the roadmap checkpoint while start refuses. Same divergence class the provisional fix closed, one granularity down; fails in the safe direction *(deferred 2026-08-03T21:08:42Z · guided-by-default/gatekeeper)*
