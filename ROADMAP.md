...
- **acceptance-skip-parser-breadth** — behave is classified as an acceptance runner but gets no summary parser in v1 (nor pytest-bdd/RSpec/Jest/Playwright), so those projects sit permanently in the undetermined-warning bucket *(deferred 2026-07-30T12:12:15Z · truthful-validators/planner)*
- **self-validate-acceptance-report-shape** — Centinela's own 'go test ./... -coverprofile' emits no skip information, so its own acceptance gate is permanently undetermined; choosing -json/-v/a separate acceptance command interacts with the single-run coverage-profile reuse *(deferred 2026-07-30T12:12:15Z · truthful-validators/planner)*
- **validate-command-verdicts-in-machine-output** — the new three-valued per-command validate verdict (Warn for an undetermined acceptance report, Fail for detected skips) is rendered only in the terminal; centinela verdict and pr-gate carry gate results and claim checks but no validate-command line, so a CI consumer reading the machine output cannot see it *(deferred 2026-07-30T13:08:03Z · truthful-validators/senior-engineer)*
- **acceptance-verbose-terminator-attribution-steal** — parseGoVerbose flushes its pending block at the first line matching ^(ok|FAIL|?)\s+\S+\s, so any unindented go-test terminator a test echoes from a subprocess re-attributes that package's skips to a foreign package; under ScopeMixed the real acceptance skips are then discarded and the run renders green *(deferred 2026-07-30T18:45:59Z · truthful-validators/gatekeeper)*
- **acceptance-tier-attribution-path-hardcoded** — AcceptancePath is the hardcoded, case-sensitive literal tests/acceptance, so under a whole-repo ScopeMixed command (go test ./...) a project whose acceptance tier sits anywhere else — features/, test/acceptance, e2e/, or a capitalised Tests/Acceptance — has skip and 0-scenario detection silently disarmed: the run renders a plain green tick with no note, no warning and no config knob, while feature-brief AC2 states without carve-out that a detected skip makes the command fail *(deferred 2026-07-31T08:08:05Z · truthful-validators/gatekeeper)*
