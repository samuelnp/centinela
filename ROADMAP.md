...
- **codex-claude-role-agent-registry** — Only OpenCode has a per-role agent registry; Codex and the Claude harness emit no role definitions — make role definitions a harness-adapter capability *(deferred 2026-07-29T09:03:04Z · unified-plan-specialist/big-thinker)*
- **managed-agent-retirement-sweep** — mergeOpenCodeAgents only adds keys, so a retired managed agent lingers in every existing opencode.json forever; needs a marked-managed removal path *(deferred 2026-07-29T09:03:04Z · unified-plan-specialist/big-thinker)*
- **prompt-doc-budget-ratchet** — The per-file 130-line prompt-doc budget does not express the invariant it protects (total prompt surface) and penalizes legitimate consolidation; convert to an aggregate ratchet *(deferred 2026-07-29T09:03:04Z · unified-plan-specialist/big-thinker)*
- **statusline-plan-role-display** — Two spec scenarios claim the statusline/status displays the plan-step role (planner, or the legacy pair); neither surface has a role field, so the scenarios are unimplemented and their acceptance tests assert a different property *(deferred 2026-07-29T12:28:38Z · unified-plan-specialist/gatekeeper)*
- **revise-to-plan-sheds-no-evidence** — revise --to plan reports '0 evidence artifacts invalidated' and leaves plan evidence intact, so complete immediately re-passes with stale evidence; InvalidationTargets' plan branch is unreachable because plan is always step index 0 *(deferred 2026-07-29T12:28:38Z · unified-plan-specialist/gatekeeper)*
- **legacy-plan-key-mixed-form-precedence** — A big-thinker tier plus a feature-specialist override table alias into one planner entry where the override wins, silently dropping the configured tier on the runners it does not cover *(deferred 2026-07-29T12:28:38Z · unified-plan-specialist/gatekeeper)*
- **planner-skeleton-prompt-header-drift** — companion_skeletons planner headers (Acceptance Criteria (Gherkin), Edge Cases) do not match planner-prompt.md (Gherkin Scenarios, Deferred Findings) despite a comment claiming they do *(deferred 2026-07-29T12:28:38Z · unified-plan-specialist/gatekeeper)*
- **stale-spec-text-after-role-retirement** — specs/promote-orchestration-agents.feature still asserts big-thinker-prompt.md and feature-specialist-prompt.md 'should exist' (now deleted), and specs/deferred-findings-roadmap-capture.feature:272 'All eight role prompts' lost its acceptance marker when the test comment was renamed to 'All role prompts'; the diff-aware warn-only traceability gate cannot see either *(deferred 2026-07-29T13:21:28Z · unified-plan-specialist/gatekeeper)*
- **brownmap-roadmap-source-role-retired** — internal/brownmap/{baseline,gaps}.go still stamp roadmap Source.Role="big-thinker" on newly generated brownfield entries, so fresh records attribute provenance to a retired role — the brief's 'references updated, no orphaned role strings' edge case is unmet *(deferred 2026-07-29T13:21:29Z · unified-plan-specialist/gatekeeper)*
- **handoff-chain-unvalidated** — ValidateEvidence only checks handoffTo is non-empty, but planner-prompt.md, every other role prompt and evidence-contract.md all state under 'validator will check' that handoffTo MUST be a specific next role; a planner evidence with handoffTo "banana" completes the plan step *(deferred 2026-07-29T14:40:06Z · unified-plan-specialist/gatekeeper)*
- **planner-stub-on-legacy-workflow-unguarded** — EnsureRoleAllowed guards only the retired-role direction; 'centinela evidence init <unpinned-feature> planner' succeeds and writes a stub the legacy plan gate can never accept, contradicting the function's own stated invariant that the CLI never offers a stub the gate would refuse *(deferred 2026-07-29T14:40:06Z · unified-plan-specialist/gatekeeper)*
- **stamp-validates-commands-schema** — artifact stamp should validate the centinela:verification commands schema (argv arrays, exitCode, durationMs) and fail loudly, so a malformed verifier record dies at stamp time instead of costing a complete-gate round-trip *(deferred 2026-07-29T16:36:24Z · unified-plan-specialist/gatekeeper)*
