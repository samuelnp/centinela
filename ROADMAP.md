...
- **hook-context-panel-diet** — Measured: the per-prompt hook emits 23 lines/1538 bytes, of which the roadmap summary token-diet quiets is only 1 line/~56 bytes (~4%); the ACTIVE WORKFLOWS panel and the brief/edge-case/changelog nudge panels are the remaining ~96% and re-render unchanged on every prompt — extend the same session+digest seam to them *(deferred 2026-07-29T17:13:45Z · token-diet/planner)*
- **code-step-cannot-repair-stale-tier-tests** — The prewrite hook blocks Write/Edit under tests/** during the code step, but a code-step change routinely invalidates frozen literals in existing tests/{unit,integration,acceptance} files (token-diet slice D updated 8 such files the plan did not list). The step cannot repair the suite it just broke without scripted edits that bypass the hook; policy should allow modifying EXISTING tests/ files during code while still blocking creation of new ones. *(deferred 2026-07-29T18:26:18Z · token-diet/senior-engineer)*
- **model-capability-spec-scenario-uncovered** — token-diet added 'Scenario Outline: The capability map retains every retired pinned model id' to specs/model-capability-profiles.feature with no // Scenario: marker in any tests/acceptance file carrying that spec's Acceptance header; the behaviour is tested under the token-diet header instead, so the scenario is untraceable and the warn-level spec-traceability gate lets it merge *(deferred 2026-07-29T21:39:28Z · token-diet/gatekeeper)*
- **legacy-model-specs-have-no-scenario-markers** — specs/configurable-model-routing.feature (16 scenarios) and specs/configurable-subagent-models.feature (12) have acceptance files with an // Acceptance: header but zero // Scenario: markers, so all 28 are uncovered by the traceability matcher; pre-existing but pulled into the diff-aware gate's scope whenever those specs are edited *(deferred 2026-07-29T21:39:28Z · token-diet/gatekeeper)*
- **plan-empty-inputs-message-masked** — specs/token-diet.feature:72 and brief edge case E4 claim an empty plan-evidence inputs list fails 'naming both required paths', but via centinela evidence validate the generic 'incomplete evidence fields' check fires first and short-circuits, so the operator never sees 'missing feature-doc snapshot inputs' nor either path name; fail-closed, message just unreachable from the CLI *(deferred 2026-07-29T21:39:38Z · token-diet/gatekeeper)*
- **summary-digest-hashes-nonschedulable-phases** — roadmap.SummaryDigest hashes all phases including Backlog/Baseline while the rendered line derives from r.Summary() which excludes them, so filing any deferred finding forces a re-render of a byte-identical roadmap line; fail-open and harmless but it defeats the dedup in exactly the gatekeeper-deferral workflow that touches the roadmap most *(deferred 2026-07-29T21:39:38Z · token-diet/gatekeeper)*
