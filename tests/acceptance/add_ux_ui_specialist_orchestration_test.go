...
func TestCompleteFailsWithoutUXEvidenceForUserFacingCode(t *testing.T) {
	d := t.TempDir()
	o, _ := os.Getwd()
	...
	if !strings.Contains(string(out), "ux-ui-specialist") {
		t.Fatalf("expected missing ux-ui-specialist details, got: %s", out)
	}
}
